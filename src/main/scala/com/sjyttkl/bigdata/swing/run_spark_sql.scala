...
object run_spark_sql {
  ...
  val QUERY_KEYWORD =
    """
      |select
      |utdid as user_id,   ---用户id
      |buyer_id,  -- 买家id
      |keyword as query  --搜索词
      |from hzsearch.dwd_log_byr_view_itemtab_record_di
      |where pt>='%1$s'
      |and view_pv>= 5
      |""".stripMargin
...
class SparkOperation(appName: String) {
  ...
  def insert_overwrite_table_repartitions(table_name: String, result: DataFrame, partitions: Int): Unit = {
    println("准备插入数据了。。。。。")
    var pt = getYersterday()
  ...
  //可以对分区进行合并，可以合并成一个。参考https://blog.csdn.net/weixin_37944880/article/details/86694829 ,  https://blog.csdn.net/qq_39160721/article/details/82387328
  def insert_overwrite_table_2repartitions(table_name: String, result: DataFrame, serviceid: String, partitions: Int): Unit = {
    println("准备插入数据了。。。。。")
    var pt = getYersterday()
    //var select_val = spark.sql("select * from tmp_result")
    val result_temp = result.repartition(partitions).persist() //分区保留100个
    result_temp.createOrReplaceTempView("tmp_result")
    var sql = s"insert overwrite table $table_name partition(pt='${pt}', serviceid = '${serviceid}') select * from tmp_result "
    spark.sql(sql) //.show(20)
    println("数据导入完毕......")
  }
